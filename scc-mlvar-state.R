...
# df_state <- df_state %>% mutate(pct_testing_shortage=pct_could_not_get_tested+pct_tested_no_result)
df_state <- df_state %>% mutate(pct_testing_shortage=pct_could_not_get_tested)

# Remove states that were hit by COVID-19 in the beggining of the pandemic
# df_state <- df_state %>% filter(!state_code %in% c("ct","dc","de","ma","nh","nj","ny","pa","ri"))
...
df_state_agg <- df_state %>%
  ...
  summarise(total_n=sum(summed_n),
            ...
            total_cases_prop=max(cases_cum_prop))
...
# df_statedf_state %>% filter(state_code %in% df_state_agg$state_code)

### Transform to wide format
selected_vars <- c("pct_cmnty_cli", "pct_worked_outside_home", "pct_avoid_contact",
                   "pct_did_not_try_to_get_tested", "pct_testing_shortage", "pct_multiple_medical_conditions",
                   "StringencyIndex",
                   "completely_home_prop",
                   "cases_prop")
# selected_vars <- c(selected_vars, colnames(df_state)[18:30], "pct_cli")
...
df_state_wide <- pivot_wider(df_state,
                             id_cols = c("date", "state_code", "StringencyIndex",
                                         "completely_home_prop", "cases_prop"),
...
# Aggregate by weeks
# library(ISOweek)
# df_state_wide <- df_state_wide %>% group_by(state_code, week = ISOweek(date)) %>%
#   summarise_all(mean) %>% 
#   dplyr::select(-date) %>%
#   rename(date=week)
...
### --- PLOT 8 SCATTER PLOT:
# StringencyIndex ---> pct_cmnty_cli_overall VS pct_avoid_contact_overall ---> pct_cmnty_cli_overall
