...
# Plot
ggplot(df_plot, aes(x = AttendedPublicEvent.CLI, y = WearMask.CLI)) +
  geom_point(size=5, alpha=0.75) +
  geom_text_repel(aes(label = country), size = 5) + 
  labs(x = "AttendedPublicEvent.CLI",
       y = "WearMask.CLI") + 
...
# --- PLOT: Compute confidence intervals for random effects >>>>>>>>>>>>>>>>>>>>>>>
library(lme4)
...
df_plot <- res_full %>% dplyr::select(V3_mean, V4_mean, V3_signif, V4_signif)
df_plot$country_agg <- rownames(df_plot)
df_plot <- df_plot %>% rename(Masks.CLI=V3_mean, Events.CLI=V4_mean)
df_plot <- df_plot %>% left_join(df_stats)

# Plot 1
ggplot(df_plot, aes(x = total_cases_prop, y = Masks.CLI)) +
  geom_point(size=5, alpha=0.75) +
...
# Plot 2
ggplot(df_plot, aes(x = total_cases_prop, y = Events.CLI)) +
  geom_point(size=5, alpha=0.75) +
  geom_text_repel(aes(label = country_agg), size = 3) + 
  labs(x = "total_cases_prop",
       y = "Events.CLI") + 
